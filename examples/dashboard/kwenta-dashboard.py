...
def get_account_info(wallet_address):
    url = f'https://api.thegraph.com/subgraphs/name/kwenta/optimism-perps'
...
st.set_page_config(layout='wide', page_title="Kwenta-V2",
...
if (endpoint == "Solana"):
    ...
    token = "SOL"
    account_data = get_account_info(wallet_address)[
        'data']['futuresPositions'][0]
    # st.write(account_data)
    col1, col2, col3, col4 = st.columns(4)
    current_position = get_current_positions(wallet_address)
    # general Account Data
    if current_position['size'] == 0:
        col1.metric("Current Position",
                    current_position['size'], "0", delta_color="off")
        col2.metric("Entry Price", "0", "0", delta_color="off")
        col3.metric("Current Price", get_current_asset_price()
                    ['usd'], "0", delta_color="off")
        col4.metric("Current Margin", (get_accessible_margin(
            wallet_address)['readable_amount']), delta_color="off")
    else:
        col1.metric("Current Position", int(current_position['size'])/(
            10**18), f"PNL: {((get_current_asset_price()['usd'])-(int(account_data['entryPrice'])/(10**18)))*int(current_position['size'])/(10**18)}", delta_color="off")
        col2.metric("Entry Price", int(account_data['entryPrice'])/(10**18), ((
            get_current_asset_price()['usd'])-(int(account_data['entryPrice'])/(10**18))), delta_color="off")
        col3.metric("Current Price", get_current_asset_price()
                    ['usd'], "0", delta_color="off")
        col4.metric("Current Margin", (get_accessible_margin(
            wallet_address)['readable_amount']), delta_color="off")
    ...
    with col1:
        if current_position['size'] == 0:
            close_button = st.button(
                "Close Position", disabled=True, key="close_pos", use_container_width=True)
        else:
    ...
    with tcol1:
        ...
        if 'long_stop_limit' not in st.session_state:
            st.session_state.long_stop_limit = get_current_asset_price()[
                'usd']*(0.96)
        ...
        long_amount = st.number_input("Margin Amount (Leave at 0 to use leverage multiplier)", value=st.session_state.long_amount,
                                      min_value=0.0, max_value=get_leveraged_amount(24.7, wallet_address)['max_asset_leverage']/(10**18), step=10.0, key="long_amount")
        # Slider for liquidation multiplier
        long_multiplier = st.number_input(
            "Liquidation Multiplier", value=st.session_state.long_multiplier, step=1.0, key="long_multiplier")
        # Limit order dropdown
        long_order_type = st.selectbox(
            "Order Type", ["Buy_Limit_Order", "Sell_Limit_Order"], key="long_order_type")
        # Amount input for limit order
        long_limit = st.number_input(
            "Limit Price", value=st.session_state.long_limit, step=1.0, key="long_limit")
        # Stop limit input box
        long_stop_limit = st.number_input(
            "Stop Limit Price", value=st.session_state.long_stop_limit, step=1.0, key="long_stop_limit")
        refresh_order_data = st.button('Refresh Order Data', on_click=refresh_values, args=(
            long_amount, long_multiplier, long_order_type, long_limit, long_stop_limit, ), use_container_width=True, key="refresh_long_values")
        with st.form(key='long_trade'):
            st.subheader("Long Trade Data")
            long_price_percent_diff = (
                (long_limit/get_current_asset_price()['usd'])-1)*100
            if (long_amount == 0.0) and (long_multiplier > 0.0):
                long_amount = long_multiplier * \
                    (get_accessible_margin(wallet_address)['readable_amount'])
            long_pnl_wo_fees = (
                ((long_limit/get_current_asset_price()['usd'])-1)*long_amount)
            long_order_df = pd.DataFrame([{"long_amount": long_amount, "long_multiplier": long_multiplier, "long_order_type": long_order_type,
                                         "long_limit": long_limit, "long_stop_limit": long_stop_limit, ",long_percent_diff": long_price_percent_diff, "pnl_w/o_fees": long_pnl_wo_fees}])
    ...
    with tcol2:
        ...
        if 'short_stop_limit' not in st.session_state:
            st.session_state.short_stop_limit = get_current_asset_price()[
                'usd']*(1.04)
        ...
        short_amount = st.number_input("Margin Amount (Leave at 0 to use leverage multiplier)", value=st.session_state.short_amount,
                                       min_value=0.0, max_value=get_leveraged_amount(24.7, wallet_address)['max_asset_leverage']/(10**18), step=10.0, key="short_amount")
        # Slider for liquidation multiplier
        short_multiplier = st.number_input(
            "Liquidation Multiplier", value=st.session_state.short_multiplier, step=1.0, key="short_multiplier")
        # Limit order dropdown
        short_order_type = st.selectbox(
            "Order Type", ["Buy_Limit_Order", "Sell_Limit_Order"], key="short_order_type")
        # Amount input for limit order
        short_limit = st.number_input(
            "Limit Price", value=st.session_state.short_limit, step=1.0, key="short_limit")
        # Stop limit input box
        short_stop_limit = st.number_input(
            "Stop Limit Price", value=st.session_state.short_stop_limit, step=1.0, key="short_stop_limit")
        refresh_order_data = st.button('Refresh Order Data', on_click=refresh_values, args=(
            short_amount, short_multiplier, short_order_type, short_limit, short_stop_limit, ), use_container_width=True, key="refresh_short_values")
        with st.form(key='short_trade'):
            st.subheader("Short Trade Data")
            short_price_percent_diff = (
                (short_limit/get_current_asset_price()['usd'])-1)*100
            if (short_amount == 0.0) and (short_multiplier > 0.0):
                short_amount = short_multiplier * \
                    (get_accessible_margin(wallet_address)['readable_amount'])
            short_pnl_wo_fees = (
                ((short_limit/get_current_asset_price()['usd'])-1)*short_amount)*-1
            short_order_df = pd.DataFrame([{"short_amount": short_amount, "short_multiplier": short_multiplier, "short_order_type": short_order_type,
                                          "short_limit": short_limit, "short_stop_limit": short_stop_limit, "short_percent_diff": short_price_percent_diff, "pnl_w/o_fees": short_pnl_wo_fees}])
    ...
    with st.expander("Plotly Graph - KWENTA API"):
        st.plotly_chart(get_candlestick_plot_simple(
            candles_df, "Solana"), use_container_width=True)
    components.html(
        """
